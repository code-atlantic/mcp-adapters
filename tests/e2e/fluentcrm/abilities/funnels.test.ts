...
describe("FluentCRM Funnels", () => {
  ...
  describe("List Funnels", () => {
    ...
    it("should search by title", async () => {
      ...
      expect(result.success).toBe(true);
      expect(
        result.data.funnels.some((f: any) => f.title.includes(searchTerm)),
      ).toBe(true);
    });
  ...
  describe("Test Funnel Conditions", () => {
    ...
    beforeAll(async () => {
      ...
      const subscribedResult = await mcp.callTool(
    ...
    it("should test subscription status condition", async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.data.test_results.conditions).toHaveProperty(
        "subscription_status",
      );
    });
    ...
    it("should test run_only_once condition", async () => {
      ...
      if (result.data.test_results.conditions.run_only_once) {
        expect(
          result.data.test_results.conditions.run_only_once,
        ).toHaveProperty("enabled");
        expect(
          result.data.test_results.conditions.run_only_once,
        ).toHaveProperty("already_processed");
        expect(
          result.data.test_results.conditions.run_only_once,
