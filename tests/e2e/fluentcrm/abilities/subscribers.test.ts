...
describe('FluentCRM Subscribers', () => {
	...
	describe('Create Subscriber', () => {
		...
		it('should reject duplicate email address', async () => {
			...
			const duplicateResult = await mcp.callTool('fluentcrm-create-subscriber', {
				email,
			});
	...
	describe('List Subscribers', () => {
		...
		it('should handle all orderby options', async () => {
			const orderbyFields = ['id', 'email', 'first_name', 'last_name', 'created_at', 'updated_at'];
	...
	describe('Bulk Import Subscribers', () => {
		it('should import multiple subscribers', async () => {
			const subscribers = [
				{ email: generateTestEmail(), first_name: 'Bulk1', status: 'subscribed' },
	...
	describe('Merge Subscribers', () => {
		it('should merge two subscribers', async () => {
			...
			expect(result.success).toBe(true);
			expect(result.data.primary_subscriber_id).toBe(primary.data.subscriber.id);
		...
		it('should merge tags from merge subscribers to primary', async () => {
			...
			const hasTag = getResult.data.subscriber.tags.some((t: any) => t.id === tagResult.data.tag.id);
		...
		it('should merge lists from merge subscribers to primary', async () => {
			...
			expect(getResult.success).toBe(true);
			const hasList = getResult.data.subscriber.lists.some((l: any) => l.id === listResult.data.list.id);
	...
	describe('Edge Cases and Boundary Conditions', () => {
		...
		it('should handle phone number formats', async () => {
			const phoneFormats = ['+1-555-0123', '(555) 123-4567', '555.123.4567', '+44 20 1234 5678'];
