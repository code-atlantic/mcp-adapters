...
import { MCPClient, TEST_CONFIG, generateTestTitle, generateTestEmail } from '../../utils/mcp-client';
...
describe('FluentCRM Lists', () => {
	...
	describe('List Lists', () => {
		...
		it('should search lists by title', async () => {
			...
			expect(searchResult.data.lists.some((list: any) => list.title === uniqueTitle)).toBe(true);
		});
	...
	describe('Get List Subscribers', () => {
		...
		it('should handle all valid status enum values', async () => {
			const statuses = ['subscribed', 'unsubscribed', 'pending', 'bounced', 'complained'];
	...
	describe('Duplicate List', () => {
		...
		it('should preserve original list description in duplicate', async () => {
			...
			expect(result.success).toBe(true);
			expect(result.data.new_list.description).toBe(result.data.new_list.description);
	...
	describe('Merge Lists', () => {
		...
		it('should generate descriptive description for merged list', async () => {
			...
			expect(result.success).toBe(true);
			expect(result.data.target_list.description).toContain('Merged list from:');
