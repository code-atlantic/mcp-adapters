...
class Users extends BaseAbility {
    ...
    public function execute_get_board_users(array $args): array {
        try {
            ...
            $relations = $relationModel->where('object_id', $board_id)
    ...
    public function execute_add_board_member(array $args): array {
        try {
            ...
            $relationModel = new \FluentBoards\App\Models\Relation();
            $existing_relation = $relationModel->where('object_id', $board_id)
                                              ->where('object_type', 'board')
    ...
    public function execute_remove_board_member(array $args): array {
        try {
            ...
            // Find the relation
            $relationModel = new \FluentBoards\App\Models\Relation();
            $relation = $relationModel->where('object_id', $board_id)
                                     ->where('object_type', 'board')
                                     ->where('foreign_id', $user_id)
                                     ->where('foreign_type', 'user')
                                     ->first();
    ...
    public function execute_get_user_info(array $args): array {
        try {
            ...
            $relationModel = new \FluentBoards\App\Models\Relation();
            $board_relations = $relationModel->where('foreign_id', $user_id)
                                            ->where('foreign_type', 'user')
                                            ->where('object_type', 'board')
                                            ->get();
    ...
    public function execute_get_all_users(array $args): array {
        try {
            ...
            foreach ($user_query->results as $user) {
                ...
                $relationModel = new \FluentBoards\App\Models\Relation();
                $board_count = $relationModel->where('foreign_id', $user->ID)
                                            ->where('foreign_type', 'user')
                                            ->where('object_type', 'board')
                                            ->count();
        ...
        try {
            ...
            foreach ($operations as $operation) {
                ...
                try {
                    ...
                    $relationModel = new \FluentBoards\App\Models\Relation();
                    $existing = $relationModel->where('object_id', $board_id)
                                             ->where('object_type', 'board')
                                             ->where('foreign_id', $user_id)
                                             ->where('foreign_type', 'user')
                                             ->first();
    ...
    public function execute_get_user_tasks(array $args): array {
        try {
            ...
            $query = $taskModel->whereHas('assignees', function($q) use ($user_id) {
    ...
    public function execute_get_user_activities(array $args): array {
        try {
            ...
            if (class_exists('\FluentBoards\App\Models\Comment')) {
                ...
                $comments = $commentModel->where('created_by', $user_id)
                                        ->with('task')
            ...
            $taskModel = new \FluentBoards\App\Models\Task();
            $tasks = $taskModel->where('created_by', $user_id)
                              ->orderBy('created_at', 'DESC')
                              ->limit(10)
                              ->get();
    ...
    public function execute_get_user_boards(array $args): array {
        try {
            ...
            $relationModel = new \FluentBoards\App\Models\Relation();
            $relations = $relationModel->where('foreign_id', $user_id)
                                      ->where('foreign_type', 'user')
                                      ->where('object_type', 'board')
                                      ->get();
