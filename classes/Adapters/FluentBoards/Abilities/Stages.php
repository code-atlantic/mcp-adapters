...
class Stages extends BaseAbility {
    ...
    private function register_list_stages(): void {
        wp_register_ability('fluentboards_list_stages', [
    ...
    private function register_create_stage(): void {
        wp_register_ability('fluentboards_create_stage', [
    ...
    private function register_update_stage(): void {
        wp_register_ability('fluentboards_update_stage', [
    ...
    private function register_delete_stage(): void {
        wp_register_ability('fluentboards_delete_stage', [
    ...
    private function register_restore_stage(): void {
        wp_register_ability('fluentboards_restore_stage', [
    ...
    private function register_reorder_stages(): void {
        wp_register_ability('fluentboards_reorder_stages', [
    ...
    private function register_move_all_tasks(): void {
        wp_register_ability('fluentboards_move_all_tasks', [
    ...
    private function register_archive_all_tasks(): void {
        wp_register_ability('fluentboards_archive_all_tasks', [
    ...
    private function register_get_archived_stages(): void {
        wp_register_ability('fluentboards_get_archived_stages', [
    ...
    private function register_sort_stage_tasks(): void {
        wp_register_ability('fluentboards_sort_stage_tasks', [
    ...
    private function register_get_stage_positions(): void {
        wp_register_ability('fluentboards_get_stage_positions', [
    ...
    public function execute_list_stages(array $args): array {
        try {
            ...
            $query = $stageModel->where('board_id', $board_id);
    ...
    public function execute_create_stage(array $args): array {
        try {
            ...
            if ($position === null) {
                $stageModel = new \FluentBoards\App\Models\Stage();
                $lastStage = $stageModel->where('board_id', $board_id)
                                      ->whereNull('archived_at')
                                      ->orderBy('position', 'DESC')
                                      ->first();
                $position = $lastStage ? $lastStage->position + 1 : 1;
            }
            ...
            $stageModel = new \FluentBoards\App\Models\Stage();
            $stage = $stageModel->create($stage_data);
    ...
    public function execute_update_stage(array $args): array {
        try {
            ...
            $stageModel = new \FluentBoards\App\Models\Stage();
            $stage = $stageModel->where('id', $stage_id)
            ...
            if (!empty($update_data)) {
                ...
                $stage = $stageModel->find($stage_id); // Refresh
            }
    ...
    public function execute_reorder_stages(array $args): array {
        try {
            ...
            $existing_stages = $stageModel->where('board_id', $board_id)
            ...
            foreach ($stage_ids as $position => $stage_id) {
                $stage = $stageModel->find($stage_id);
                if ($stage) {
    ...
    public function execute_move_all_tasks(array $args): array {
        try {
            ...
            $stageModel = new \FluentBoards\App\Models\Stage();
            $stages = $stageModel->where('board_id', $board_id)
            ...
            $tasks = $taskModel->where('board_id', $board_id)
    ...
    public function execute_sort_stage_tasks(array $args): array {
        try {
            ...
            // Get and sort tasks
            $taskModel = new \FluentBoards\App\Models\Task();
            $query = $taskModel->where('board_id', $board_id)
                             ->where('stage_id', $stage_id)
                             ->whereNull('archived_at');
