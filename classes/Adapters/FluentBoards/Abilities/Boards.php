...
class Boards extends BaseAbility {
	...
	public function execute_update_board( array $args ): array {
		try {
			...
			if ( ! empty( $update_data ) ) {
				...
				$board = $board_model->find( $board_id ); // Refresh
			}
	...
	public function execute_restore_board( array $args ): array {
		try {
			...
			$board_model = new \FluentBoards\App\Models\Board();
			$board       = $board_model->withTrashed()->find( $board_id );
	...
	public function execute_duplicate_board( array $args ): array {
		try {
			...
			$board_model    = new \FluentBoards\App\Models\Board();
			$original_board = $board_model->with( [ 'stages', 'users' ] )->find( $board_id );
			...
			// Check user access
			$user_id = get_current_user_id();
			if ( ! $this->can_access_board( $original_board, $user_id ) ) {
			...
			$new_board = $board_model->create( $new_board_data );
