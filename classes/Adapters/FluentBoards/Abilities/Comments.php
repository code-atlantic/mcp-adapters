...
class Comments extends BaseAbility {
	...
	public function execute_get_comments( array $args ): array {
		try {
			...
			$comments      = $comment_model->where( 'object_id', $task_id )
	...
	public function execute_add_comment( array $args ): array {
		try {
			...
			if ( 'reply' === $comment_type && $parent_id ) {
				$comment_model  = new \FluentBoards\App\Models\Comment();
				$parent_comment = $comment_model->find( $parent_id );
				if ( ! $parent_comment ) {
			...
			$comment = $comment_model->create( $comment_data );
	...
	public function execute_update_comment( array $args ): array {
		try {
			...
			if ( $comment->created_by !== $user_id && ! current_user_can( 'manage_options' ) ) {
	...
	public function execute_update_reply( array $args ): array {
		try {
			...
			$comment_model = new \FluentBoards\App\Models\Comment();
			$reply         = $comment_model->find( $reply_id );
			...
			if ( $reply->created_by !== $user_id && ! current_user_can( 'manage_options' ) ) {
	...
	public function execute_delete_comment( array $args ): array {
		try {
			...
			// Get comment
			$comment_model = new \FluentBoards\App\Models\Comment();
			$comment       = $comment_model->with( 'replies' )->find( $comment_id );
